...
java -javaagent:/sls-mall/opentelemetry-javaagent-all.jar -Dotel.exporter.otlp.endpoint=${ENDPOINT} \
  ...
  -Dotel.resource.attributes=deployment.environment=${DEPLOYMENT_ENVIRONMENT},ervice.name=${SERVICE_NAME},service.version=${SERVICE_VERSION},host.name=${SERVICE_HOST} -jar app.jar